...
set out_res = ${OUT_DIR}/spline/res3D/Precursor${precursor}.pdf
set out_gof = ${OUT_DIR}/spline/gof/Precursor${precursor}.txt
set out_models = ${OUT_DIR}/spline/model/Precursor${precursor}.xml

set param = "IsotopeSpline('"${max_mass}"','"${S}"','"${precursor}"','"${fragment}"','"${file}"','"${out_res_hist}"','"${out_scatter}"','"${out_res}"','"${out_gof}"','"${out_models}"');quit force"
