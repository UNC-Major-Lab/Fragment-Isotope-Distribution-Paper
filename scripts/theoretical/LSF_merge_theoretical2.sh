...
python ${SOURCE_DIR}/scripts/theoretical/mergeHistogram.py $IN_DIR $BIN_SIZE_CHISQUARE "merged_precursor_scores_" 1 10 F > ${IN_DIR}"/precursor_scores.txt"
python ${SOURCE_DIR}/scripts/theoretical/mergeHistogram.py $IN_DIR $BIN_SIZE_RESIDUAL "merged_precursor_residuals_" 1 10 F > ${IN_DIR}"/precursor_residuals.txt"
